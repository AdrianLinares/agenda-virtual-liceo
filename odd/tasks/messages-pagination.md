...
- Add behavior-focused component tests for page navigation, date filtering in both tabs, empty/boundary pages, and resetting pagination when criteria change.
...
- Test runner: `pnpm run test:ci` (authoritative Vitest run); focused command: `pnpm test -- src/pages/__tests__/MensajesPage.spec.tsx`.
...
- [x] W1 — Add strict-TDD coverage and implement paginated date filtering for received and sent messages.
  - Acceptance: both tabs request only the current page, filter `created_at` to the inclusive selected dates, expose navigation based on the filtered result count, and reset to the first page when tab/date criteria change.
  - Verification: `pnpm lint` passed; `pnpm build` passed (Vite emitted only the existing stale Browserslist data notice); `pnpm run test:ci` passed (18 files, 91 tests); focused `pnpm exec vitest run src/pages/__tests__/MensajesPage.spec.tsx` passed (1 file, 4 tests).
...
- W1 now uses 20-row server pagination with exact filtered counts and date filters. Date boundaries are local-calendar midnight instants; the end bound is exclusive midnight of the following local date so the selected end date is fully included.
...
## Next step
W1 is committed and verified. Keep W2 blocked pending exact remote authorization; leave `.atl/skill-registry.md` and `.codegraph/` untouched. Engram mirror resynchronization is pending.
